...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int ta_i = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(ta_i) {}
    };
...
public:
    ...
    int kesimo(int k) const {
        return kesimo(k, raiz);
    }

protected:

    int kesimo(int k, Link a) const {
        //k es la posicion del numero
        if (a == nullptr) return 0;
        else {
            //si la raiz justo es la que buscamos
            if (k == a->tam_i) {
                return a->elem;
            }
            //si k esta en el hijo izquierdo
            else if (menor(k, a->tam_i)) {
                return kesimo(k, a->iz);
            }
            //si k esta en el hijo derecho
            else if (menor(a->tam_i, k)) {
    ...
    bool inserta(T const& e, Link& a) {
        ...
        else // el elemento e ya esta en el Ãrbol
            crece = false;
        return crece;
    }
    ...
    void rotaDer(Link& r2) {
        ...
        // Si rota hacia la derecha, la raiz no tendra hijo izquierdo
        r2->tam_i -= r1->tam_i;
    ...
    void rotaIzq(Link& r1) {
        ...
        // Si rota hacia la izquierda, r2 tendra tam_i de r1 y mas uno que representa r1
        r2->tam_i += r1->tam_i;
